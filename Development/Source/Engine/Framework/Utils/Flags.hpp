...
template <typename T>
inline hd::util::Flags<T> operator|(hd::util::Flags<T> const& first, hd::util::Flags<T> const& second)
{
    hd::util::Flags<T> result;
    result.SetBitfield(first.GetBitfield() | second.GetBitfield());
    return result;
}

template <typename T, typename>
inline hd::util::Flags<T> operator|(T first, T second)
{
    return hd::util::Flags<T>(first) | hd::util::Flags<T>(second);
}

template <typename T, typename>
inline hd::util::Flags<T> operator|(hd::util::Flags<T> const& first, T second)
{
    return first | hd::util::Flags<T>(second);
}

template <typename T, typename>
inline hd::util::Flags<T> operator|(T first, hd::util::Flags<T> const& second)
{
    return hd::util::Flags<T>(first) | second;
}