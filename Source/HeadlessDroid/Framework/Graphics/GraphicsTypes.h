...
{
	struct BufferTag {};
	using BufferHandle = Handle<BufferTag, uint64_t, std::numeric_limits<uint64_t>::max()>;

	struct TextureTag {};
	using TextureHandle = Handle<TextureTag, uint64_t, std::numeric_limits<uint64_t>::max()>;
	...
	using BufferFlags = Flags<BufferFlagsBits>;
	...
	using TextureFlags = Flags<TextureFlagsBits>;
