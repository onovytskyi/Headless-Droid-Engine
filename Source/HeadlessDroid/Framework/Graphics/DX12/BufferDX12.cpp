...
namespace hd
{
	GfxBuffer::GfxBuffer(Device& device, ID3D12Resource* resource, uint32_t numElements, uint32_t elementSize, BufferFlags flags)
		: GfxBuffer(device, resource, D3D12_RESOURCE_STATE_COMMON, numElements, elementSize, flags)
	{
	}

	GfxBuffer::GfxBuffer(Device& device, ID3D12Resource* resource, D3D12_RESOURCE_STATES initialState, uint32_t numElements, uint32_t elementSize, BufferFlags flags)
		: m_Data{}
		  , m_HeapAllocation{}
		  , m_CBV{}
		  , m_SRV{}
	...
	GfxBuffer::GfxBuffer(Device& device, HeapAllocator::Allocation const& heapAllocation, uint32_t numElements, uint32_t elementSize, BufferFlags flags)
		: m_Data{}
	...
	void GfxBuffer::Free(Device& device)
	{
		if (m_Data.Resource)
		{
	...
	ID3D12Resource* GfxBuffer::GetNativeResource() const
	{
		return m_Data.Resource != nullptr ? m_Data.Resource : m_HeapAllocation.ResourceData->Resource;
	}

	ResourceStateTracker::StateTrackedData& GfxBuffer::GetStateTrackedData()
	{
		return m_Data.Resource != nullptr ? m_Data : *m_HeapAllocation.ResourceData;
	}

	DescriptorSRV GfxBuffer::GetCBV() const
	{
		return m_CBV;
	}

	DescriptorSRV GfxBuffer::GetSRV() const
	{
		return m_SRV;
	}

	DescriptorSRV GfxBuffer::GetUAV() const
	{
		return m_UAV;
	}

	size_t GfxBuffer::GetBaseOffset() const
	{
		return m_Data.Resource != nullptr ? 0 : m_HeapAllocation.Offset;
	}

	uint32_t GfxBuffer::GetSize() const
	{
		return m_Size;
	}

	void GfxBuffer::CreateViews(Device& device, uint32_t numElements, uint32_t elementSize, BufferFlags flags)
	{
		DescriptorManager& descriptorManager = device.GetDescriptorManager();
