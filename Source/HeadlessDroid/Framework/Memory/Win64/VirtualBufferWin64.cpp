...
namespace hd
{
	...
	{
		hdAssert(IsAligned(s_AllocationGranularity, alignInBytes), u8"Can only allocate with alignment compatible with allocation granularity.");
	...
	{
		size_t oldSizeInPages = AlignAbove(oldSizeInBytes, s_PageSize) / s_PageSize;
		size_t newSizeInPages = AlignAbove(sizeInBytes, s_PageSize) / s_PageSize;
