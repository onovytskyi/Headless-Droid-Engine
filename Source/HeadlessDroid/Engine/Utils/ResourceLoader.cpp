...
namespace hd
{
	...
	void LoadRawMesh(std::pmr::u8string const& filePath, std::pmr::vector<MeshResource>& outMeshes, std::pmr::vector<MaterialResouce>& outMaterials)
	{
		...
		for (uint32_t submeshIdx = 0; submeshIdx < outMeshes.size(); ++submeshIdx)
		{
			...
			MeshResourceVertex* vertices = hdAllocate(Scratch(), MeshResourceVertex, totalIndices);
			...
			uint32_t* remapTable = hdAllocate(Scratch(), uint32_t, totalIndices);
	...
	void CookMesh(std::pmr::u8string const& meshPath, std::pmr::u8string const& cookedMeshPath)
	{
		...
		std::pmr::vector<MeshResource> meshes{ &Scratch() };
		std::pmr::vector<MaterialResouce> materials{ &Scratch() };

		LoadRawMesh(meshPath, meshes, materials);

		CommandBuffer memoryFileStream{ MB(32) };
		...
		{
			Vectorf4& diffuseColor = memoryFileStream.Write<Vectorf4>();
			diffuseColor = materials[materialIdx].DiffuseColor;

			Vectorf4& ambientColor = memoryFileStream.Write<Vectorf4>();
			ambientColor = materials[materialIdx].AmbientColor;

			Vectorf3& specularColor = memoryFileStream.Write<Vectorf3>();
	...
	{
		std::pmr::u8string meshFileName{ fileName, &Scratch() };

		std::pmr::u8string cookedMeshFilePath{ &Scratch() };
		file::ConvertToCookedPath(meshFileName, cookedMeshFilePath);
		std::pmr::u8string cookedFileExtension{ u8".bin", &Scratch() };
		file::ReplaceExtension(cookedMeshFilePath, cookedFileExtension, cookedMeshFilePath);

#if defined(HD_ENABLE_RESOURCE_COOKING)
		std::pmr::u8string meshFilePath{ &Scratch() };
		...
		CommandBufferReader fileReader{ memoryFileStream };
		...
		if (materialCount > 0)
		{
			...
			{
				outMaterials[materialIdx].DiffuseColor = fileReader.Read<Vectorf4>();
				outMaterials[materialIdx].AmbientColor = fileReader.Read<Vectorf4>();
				outMaterials[materialIdx].SpecularColor = fileReader.Read<Vectorf3>();
	...
	void LoadImageDefault(char8_t const* filePath, PlainDataArray<std::byte>& outData, ImageResource& outImageDesc)
	{
		...
		if (stbi_is_hdr(reinterpret_cast<const char*>(filePath)))
		{
			...
			outImageDesc.Format = GraphicFormat::RGBA32Float;
		...
		else
		{
			...
			outImageDesc.Format = GraphicFormat::RGBA8UNorm;
		...
		if ((ddsHeader->ddspf.Flags & DDPF_FOURCC) == DDPF_FOURCC)
		{
			...
			else
			{
				...
				case MakeFourCC('D', 'X', 'T', '1'):
					{
						outImageDesc.Format = GraphicFormat::BC1Unorm;
						break;
					}
				...
				case MakeFourCC('D', 'X', 'T', '3'):
					{
						outImageDesc.Format = GraphicFormat::BC2Unorm;
						break;
					}
				...
				case MakeFourCC('D', 'X', 'T', '5'):
					{
						outImageDesc.Format = GraphicFormat::BC3Unorm;
						break;
					}

				case MakeFourCC('B', 'C', '4', 'U'):
					{
						outImageDesc.Format = GraphicFormat::BC4Unorm;
						break;
					}

				case MakeFourCC('B', 'C', '4', 'S'):
					{
						outImageDesc.Format = GraphicFormat::BC4Snorm;
						break;
					}

				case MakeFourCC('B', 'C', '5', 'U'):
					{
						outImageDesc.Format = GraphicFormat::BC5Unorm;
						break;
					}

				case MakeFourCC('B', 'C', '5', 'S'):
					{
						outImageDesc.Format = GraphicFormat::BC5Snorm;
						break;
					}
			...
			case 32:
				{
					if (hasAlpha)
					{
						...
						{
							outImageDesc.Format = GraphicFormat::BGRA8Unorm;
						}
						else
						{
					...
					else
					{
						...
						{
							outImageDesc.Format = GraphicFormat::BGRX8Unorm;
						}
					}
	...
	{
		PlainDataArray<std::byte> fileData{ &Scratch() };
		std::pmr::u8string filePath{ fileName, &Scratch() };
