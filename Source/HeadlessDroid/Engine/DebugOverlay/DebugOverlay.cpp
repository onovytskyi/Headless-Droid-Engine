...
namespace hd
{
	DebugOverlay::DebugOverlay(Backend& backend, Device& device, uint32_t width, uint32_t height, GraphicFormat targetFormat)
		: m_Device{ &device }
		  , m_DefaultFont{}
		  ...
		  , m_Tools{ &General() }
	{
		...
		m_GuiDrawRenderState = hdNew(Persistent(), RenderState)(backend);
		...
		m_GuiDrawRenderState->SetPrimitiveType(PrimitiveType::Triangle);
		m_GuiDrawRenderState->SetBlendType(BlendType::Alpha, BlendType::None);
	...
	DebugOverlay::~DebugOverlay()
	{
		...
		hdSafeDelete(Persistent(), m_GuiDrawRenderState);
	}

	void DebugOverlay::UploadFont(hd::CommandBuffer& commandBuffer)
	{
		ImGuiIO& imguiIO = ImGui::GetIO();
		...
		m_DefaultFont = m_Device->CreateTexture(fontTextureWidth, fontTextureHeight, 1, 1, GraphicFormat::RGBA8UNorm, TextureFlagsBits::ShaderResource,
		                                        TextureDimenstion::Texture2D, nullptr);

		GraphicCommandsStream commandStream{ commandBuffer };
		commandStream.UpdateTexture(m_DefaultFont, 0, ALL_SUBRESOURCES, fontTextureData, size_t(fontTextureWidth) * fontTextureHeight * fontTextureBPP);
	...
	void DebugOverlay::Render(TextureHandle target, CommandBuffer& graphicCommands)
	{
		GraphicCommandsStream commandStream{ graphicCommands };
		...
		if (drawData && drawData->CmdListsCount > 0)
		{
			...
			BufferHandle indexBuffer = m_Device->CreateBuffer(indexCount, uint32_t(sizeof(ImDrawIdx)), BufferFlagsBits::ShaderResource | BufferFlagsBits::Transient);
			BufferHandle vertexBuffer = m_Device->CreateBuffer(vertexCount, uint32_t(sizeof(ImDrawVert)), BufferFlagsBits::ShaderResource | BufferFlagsBits::Transient);
			...
			BufferHandle constantBuffer = m_Device->CreateBuffer(1, sizeof(m_GuiProjectionMatrix), BufferFlagsBits::ConstantBuffer | BufferFlagsBits::Transient);
			Matrix4x4 gpuGuiProjectionMatrix = MatrixTranspose(m_GuiProjectionMatrix);
			...
			commandStream.SetTopologyType(TopologyType::List);
			...
			for (int32_t cmdListIdx = 0; cmdListIdx < drawData->CmdListsCount; ++cmdListIdx)
			{
				...
				for (ImDrawCmd& renderCmd : cmdList->CmdBuffer)
				{
					...
					{
						commandStream.UseAsReadableResource(TextureHandle(renderCmd.TextureId));
						commandStream.SetRootVariable(5, m_Device->GetSRVShaderIndex(TextureHandle(renderCmd.TextureId)));
					}
					else
					{
