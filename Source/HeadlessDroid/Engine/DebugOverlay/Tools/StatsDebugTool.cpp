...
namespace hd
{
	...
	{
		float ratio = float(ToMB(used)) / float(ToMB(max));
		if (ratio <= 0.5f)
		{
			return ImVec4(0.0f, 1.0f, 0.0f, 1.0f);
		}
	...
	void StatsDebugTool::Draw()
	{
		...
		if (isOpen)
		{
			...
			ImGui::TextColored(ChooseColorForBytes(m_LocalUsage, m_LocalBudget), "VRAM %lluMb/%lluMb", ToMB(m_LocalUsage), ToMB(m_LocalBudget));
			PutSeparator();

			ImGui::TextColored(ChooseColorForBytes(m_LocalUsage, m_LocalBudget), "SysRAM %lluMb/%lluMb", ToMB(m_NonlocalUsage), ToMB(m_NonlocalBudget));
			PutSeparator();
	...
	void StatsDebugTool::Update(Timer& timer, Swapchain& swapchain, Device& device)
	{
